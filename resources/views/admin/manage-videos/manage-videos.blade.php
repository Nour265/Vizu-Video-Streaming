            ...
            <table class="min-w-full bg-gray-900 text-white rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-700 hover:bg-gray-800 transition">
                        <td class="py-3 px-4">{{ $video->VidID }}</td>
                        <td class="py-3 px-4">{{ $video->title }}</td>
                        <td class="py-3 px-4">{{ $video->user->username }}</td>
                        <td class="py-3 px-4 text-center">
                            <a href="{{ route('video.show', $video->VidID) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition">View</a>
                            <a href="{{ route('admin.videos.edit', $video->VidID) }}" class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-md transition ml-2">Edit</a>
                            <button onclick="openDeleteModal({{ $video->VidID }})" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md transition ml-2">Delete</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
<!-- Modal JavaScript -->
<script>
    function openDeleteModal(VidID) {
        document.getElementById('deleteForm').action = "/admin/videos/" + VidID;
